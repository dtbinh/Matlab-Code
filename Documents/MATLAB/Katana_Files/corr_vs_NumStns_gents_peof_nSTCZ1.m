...
DIR_NAME = ['../Data/Pseudoproxies/',num2str(window),'yrWindow/peof_nSTCZ_ts'] ; mkdir(DIR_NAME);
...
for c=1:size(CAL_WDW,1)
    ...
    [~,S_bd]= min(abs(lat--30));
    [~,N_bd]= min(abs(lat-0));
    [~,W_bd]= min(abs(lon-170));
    [~,E_bd]= min(abs(lon-260));
    corr_ts(S_bd:N_bd,W_bd:E_bd) = 0;
...
fprintf(fid,'STCZ Region has not been included\n');
