...
DIR_NAME = ['../Data/Pseudoproxies/',num2str(window),'yrWindow/pneof_ts'] ; mkdir(DIR_NAME);
...
for NUM_STNS = 3:STN_MAX
    ...
    indice_pool_p = find(abs(corr_ts)>MIN_COR & ...
                       squeeze(rc91_eof_ts_fm(1,:,:)) > 0.01 & ...
                       1 & ...
                       1 & ...
                       1                                                     );
    indice_pool_n = find(abs(corr_ts)>MIN_COR & ...
                       squeeze(rc91_eof_ts_fm(1,:,:)) < -0.01 & ...
                       1 & ...
                       1 & ...
                       1                                                     );
    indice_pool = cat(1,indice_pool_p,indice_pool_n);
    assert(length(indice_pool)>100);
    ...
    for m=1:NUM_TRIALS
        ...
        [stn_lat(m,1:floor(NUM_STNS/2)),stn_lon(m,1:floor(NUM_STNS/2))] = ...
            ind2sub(size(corr_ts),indice_pool_p(randperm(length(indice_pool_p),floor(NUM_STNS/2))));
        [stn_lat(m,(floor(NUM_STNS/2)+1):NUM_STNS),stn_lon(m,(floor(NUM_STNS/2)+1):NUM_STNS)] = ...
            ind2sub(size(corr_ts),indice_pool_n(randperm(length(indice_pool_n),NUM_STNS-floor(NUM_STNS/2))));
...
% fprintf(fid,'rc31_eof_ts_fm1 > 0.01\n');
fprintf(fid,'rc91_eof_ts_fm1 > 0.01 for 1:floor(NUM_STNS/2) Proxies, rc91_eof_ts_fm1 < -0.01 for (floor(NUM_STNS/2)+1):NUM_STNS Proxies\n');
fclose(fid);
