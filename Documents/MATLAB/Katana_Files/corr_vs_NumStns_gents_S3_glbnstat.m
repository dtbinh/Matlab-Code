...
DIR_NAME = ['../Data/Pseudoproxies/',num2str(window),'yrWindow/glb_ts_nstat_sigpcd_ideal'] ; mkdir(DIR_NAME);
...
corr_ts_allyr = nan(size(ats,2),size(ats,3),'single');
for i=S_bound:N_bound
    for j=W_bound:E_bound
        corr_ts_allyr(i,j) = corr(n34_ind,ats(:,i,j));
    end
end
...
fprintf(fid,'abs(corr_ts_allyr)>MIN_COR\n');
