...
% Plotting CPS
    ...
    title([strrep(DIR_NAME(15:end),'_','\_'), ' - Correlation percentiles of CPS\_RV for each calibration window'])
    xlabel('Number of Stations included in reconstruction'); ylabel('Correlation')
    grid on
    ylim([0 1])
    legend([HA(1:end,3);HA(1,1);HA(1,2)],'Year 1-50 median','Year 51-100 median','Year 101-150 median','Year 151-200 median',...
...
% Plotting MRV
    ...
    corr_RV_qn = quantile(temp_corr_MRV,[.05 .5 .95], 2);
    clf;
