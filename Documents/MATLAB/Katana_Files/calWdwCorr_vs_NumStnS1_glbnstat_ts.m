...
DIR_NAME = ['../Data/Pseudoproxies/',num2str(window),'yrWindow/glb_ts_nstat_sigpcd'];                                 %%%%%%%
...
for n=1:NUM_TRIALS
    [eof_stn(n,:,:),PC_stn(n,:,CAL_WDW),expvar_stn(n,:)] = caleof(squeeze(all_stn_ts(n,:,CAL_WDW))', NUM_OF_EOFS, 1);
end
...
for n=1:NUM_TRIALS
    corr_matrix = corr(n34_ind(CAL_WDW)*ones(1,NUM_STNS), squeeze(all_stn_ts(n,:,CAL_WDW))');
