...
DIR_NAME = ['../Data/Pseudoproxies/',num2str(window),'yrWindow/glb_pr_nstat_sigpcd_ideal'] ; mkdir(DIR_NAME);
...
corr_pr_allyr = nan(size(apr,2),size(apr,3),'single');
for i=S_bound:N_bound
    for j=W_bound:E_bound
        corr_pr_allyr(i,j) = corr(n34_ind,apr(:,i,j));
    end
end
...
fprintf(fid,'abs(corr_pr_allyr)>MIN_COR\n');
