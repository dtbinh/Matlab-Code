...
tic;
% Limits of box
S_lat = -60; N_lat = 60; W_lon = 0; E_lon = 60;
...
for i=W_bound:E_bound
    for j=S_bound:N_bound
        if exist(['Synth_corr_',num2str(window),'yrwin_0-120E/',num2str(lon(i)),'E',num2str(lat(j)),'N_syncorr.mat'],'file')
            load(['Synth_corr_',num2str(window),'yrwin_0-120E/',num2str(lon(i)),'E',num2str(lat(j)),'N_syncorr.mat'])
        ...
        if recalc==true
            for n=1:1000 % This will take about 6 minutes
                load(['Synth_corr_',num2str(window),'yrwin_0-120E/run',num2str(n),'syncorr.mat']);
                spot_ts(n,:) = ts_synruncorr(:,j,i);
                spot_pr(n,:) = pr_synruncorr(:,j,i);
            end
            save(['Synth_corr_',num2str(window),'yrwin_0-120E/',num2str(lon(i)),'E',num2str(lat(j)),'N_syncorr.mat'],...
                'spot_ts','spot_pr');
        end
        toc;
    end
