...
%         xlim([0 1]); ylim([0 500]); xlabel(' Percentage of non-stationary stations ');
%         % h = findobj(gca,'Type','Patch');
%         % set(h,'FaceColor',[1 1 1], 'EdgeColor','black');
%         ylabel('Number of reconstructions (out of 1000)')
%         title('Proportion of nstat stns per reconstruction');
%         subplot(2,1,1);
%         hist(all_nstat_yrs',[0:5:500]+5/2); xlim([0 200]); ylim([0 500]); grid minor;
%         % h = findobj(gca,'Type','Patch');
%         % set(h,'FaceColor',[1 1 1], 'EdgeColor','black');
...
%         saveas(gcf,['Plots/hist(nstat_nstns_nstat_avyrs)',num2str(window),'rcorwdw_grp',num2str(group_size),'_',GROUP_NAME,'.jpg'])

    end
end

%% Number of non-stationarities vs reconstruction skill
...
for c=1:size(CAL_WDW,1)
    ...
    nstat_avyrs = median(nstat_yrs,2);
    nstat_nstns = sum(nstat_yrs > ceil(0.1*(NUM_YRS-window)),2)/group_size;
    all_nstat_yrs(c,:) = nstat_avyrs;
    all_nstat_nstns(c,:) = nstat_nstns;
...
data = [all_nstat_yrs(:), corrs(:)]; 
hist3(data); xlabel('Average number of nonstationary Years'); ylabel('Correlation of reconstructions');